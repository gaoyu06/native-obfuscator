...
- Latest compiler parent XML at measurement time: **#400 (692)** (`IrCompilerTest` 685 + `CodegenModeTest` 7). This measurement adds no compiler XML.
...
- 测量时最新的编译器父级 XML：**#400 (692)**（`IrCompilerTest` 685 + `CodegenModeTest` 7）。本次测量不新增编译器 XML。
