...
- Adds the fixture-only IR admission shape
  `new-constructor-extra-local-argument-five-first-third`.
- Retains this constructor leaf in JVM bytecode:
  `NEW java/util/GregorianCalendar; DUP; ILOAD 3; ICONST_2; ILOAD 3; ICONST_4; ICONST_5; INVOKESPECIAL <init>(IIIII)V`.
...
- All three added tests passed:
  `admitsThreeImmediateReturnsWithNewExtraLocalFiveFirstThirdArgChainInputs`,
  `rewrittenThreeImmediateNewExtraLocalFiveFirstThirdArgChainInputsPassJvmVerification`,
  and
  `threeImmediateNewExtraLocalFiveFirstThirdArgChainInputsCompileAndRunWithJavaParity`.
- Disposable child XML reported `644` tests, `0` skipped, `0` failures,
  and `0` errors (`IrCompilerTest`: `637`; `CodegenModeTest`: `7`).
  These are not parent totals; the parent will discard this XML and rerun the
  gate, where the expected XML total is `637 + 7 = 644`.
- 三个新增测试均通过。一次性子代理 XML 共报告 `644` 个测试，
  `0` 跳过、`0` 失败、`0` 错误（`IrCompilerTest`：`637`；
  `CodegenModeTest`：`7`）。这不是父代理统计；父代理会丢弃此次 XML
  并重新运行门禁，预期 XML 总数为 `637 + 7 = 644`。
