...
- Adds the fixture-only IR admission shape
  `new-constructor-extra-local-argument-five-all`.
- Retains this constructor leaf in JVM bytecode:
  `NEW java/util/GregorianCalendar; DUP; ILOAD 3; ILOAD 3; ILOAD 3; ILOAD 3; ILOAD 3; INVOKESPECIAL <init>(IIIII)V`.
...
- All three added tests passed:
  `admitsThreeImmediateReturnsWithNewExtraLocalFiveAllArgChainInputs`,
  `rewrittenThreeImmediateNewExtraLocalFiveAllArgChainInputsPassJvmVerification`,
  and
  `threeImmediateNewExtraLocalFiveAllArgChainInputsCompileAndRunWithJavaParity`.
- Disposable child XML reported `638` tests, `0` skipped, `0` failures,
  and `0` errors (`IrCompilerTest`: `631`; `CodegenModeTest`: `7`).
  These are not parent totals; the parent will discard this XML and rerun the
  gate, where the expected XML total is `631 + 7 = 638`.
- 三个新增测试均通过。一次性子代理 XML 共报告 `638` 个测试，
  `0` 跳过、`0` 失败、`0` 错误（`IrCompilerTest`：`631`；
  `CodegenModeTest`：`7`）。这不是父代理统计；父代理会丢弃此次 XML
  并重新运行门禁，预期 XML 总数为 `631 + 7 = 638`。
