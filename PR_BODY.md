...
Cite `threeImmediateNewFiveArgChainInputsCompileAndRunWithJavaParity`. Child totals are discarded.
...
- `IrCompilerTest`：500 个测试，0 个失败，0 个错误，0 个跳过。
- `CodegenModeTest`：7 个测试，0 个失败，0 个错误，0 个跳过。
- 总计：507 个测试，0 个失败，0 个错误，0 个跳过。

引用 `threeImmediateNewFiveArgChainInputsCompileAndRunWithJavaParity`。子代理汇总作废。
