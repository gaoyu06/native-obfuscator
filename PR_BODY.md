...
- **(a) Scope:** Combines the benchmark harness from
  ...
  evaluator-selected JNI. The branch started at benchmark tip `7221987` and
  merged evaluator tip `52f5efb` as merge commit `bd0793f`. The report records
  every sample, median, mean, environment, checksum, generated-path evidence,
  and fallback status.
...
- **(d) Verification:** All native modes use identical warmup/iteration counts
  (5/10) and `CC=gcc CXX=g++`. Legacy and direct IR stayed on their intended
  paths. Eval rejected `USHR` at bytecode instruction 22 and fell back to
  legacy, so no eval timing is claimed. The final commands, environment, path
  evidence, and measured results are recorded in
  `docs/benchmarks/results-ir-eval-lower.md`. Focused tests and all three native
  pipelines passed. No GitHub PR was opened.
...
- **(a) 范围：** 合并 `cursor/bench-ir-vs-legacy-6d81` 的基准框架与
  ...
  evaluator-selected JNI 四种模式下的表现。本分支从基准 tip `7221987` 开始，
  并把 evaluator tip `52f5efb` 合并为 merge commit `bd0793f`。报告记录全部样本、
  中位数、平均值、环境、校验和、实际生成路径证据及 fallback 状态。
- **(b) 可直接发布？否。** 这只是窄整数切片的本机编译器/基准证据，不是发布门槛，
  也不是可移植性能结果。
- **(c) 是否需要评审？是。** 需评审分支整合、模式选择、路径分类、统一基准参数、
  校验和验证、原始样本及结论。
- **(d) 验证：** 所有 native 模式使用完全相同的预热/迭代次数和
  （5/10）及 `CC=gcc CXX=g++`。Legacy 与 direct IR 均保持预期路径；eval 因
  bytecode instruction 22 的 `USHR` 不受支持而回退到 legacy，因此不声明 eval
  timing。最终命令、环境、路径证据和实测结果记录在
  `docs/benchmarks/results-ir-eval-lower.md`。聚焦测试及三条 native pipeline
  均通过。本分支未创建 GitHub PR。
