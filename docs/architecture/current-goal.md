...
1. **Fill IR admission gaps** (current work). Known leftovers on
   `master` after #175 (bridge-path-only conditional extras) include at
   ...
   returns, other post-call work, or non-identical suffixes, extras
   still unassigned on a bridge-taking path), remaining
