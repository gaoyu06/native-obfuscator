...
1. **Fill IR admission gaps** (current work). Known leftovers on
   `master` after #179 (3+ computed/constant chain inputs) include at
   ...
   such as unlisted computed inputs, post-call work, or non-identical
   ...
   admission (#169 measurement on post-#168 master) observed no
