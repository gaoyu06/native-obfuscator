...
1. **Fill IR admission gaps** (current work). Known leftovers on
   `master` after #283 (isolated three-arg `NEW`)
   ...
   `DUP`/`<init>`, array allocation), still rejected after #274/#283, and unproven `GETFIELD` inputs (local 0, extra-local of `this`,
