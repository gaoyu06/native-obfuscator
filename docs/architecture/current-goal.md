...
1. **Fill IR admission gaps** (current work). Known leftovers on
   `master` after #302 (extra-local five-arg NEW)
   ...
   overwritten / computed extra as a `NEW` argument, float/double
   initializer arguments), still rejected after #274/#293/#294/#299, unproven extra-array `AALOAD` (computed/`INEG` extra-array store, overwrite, prior array stores, primitive array as a reference result), still rejected after #288, and unproven `GETFIELD` inputs (local 0, extra-local of `this`,
   ...
   admission (#301 measurement on post-#299 `d070653`) observed no
   leftover methods; that is not a complete JVM inventory. #297 remains
