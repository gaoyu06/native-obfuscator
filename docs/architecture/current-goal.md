...
1. **Fill IR admission gaps** (current work). Known leftovers on
   `master` after #183 (leaf-only `ISUB`/`IMUL` chain inputs) include at
   ...
   such as nested/`IDIV`/shift/bitwise computed inputs or
   non-identical suffixes, extras still unassigned on a
   bridge-taking path),
