...
| JNI native-access restrictions | 24 | Current loader calls `System.load`/`System.loadLibrary`, and transformed classes declare native methods. | JDK 24+ default is warning/allow, not a hard failure; `--illegal-native-access=deny` is available and future default denial is planned ([JEP 472](https://openjdk.org/jeps/472)). Packaging/docs must address it. |
