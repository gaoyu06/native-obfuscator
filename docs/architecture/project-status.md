...
Last updated after landing one `LNEG` over a declared long `LLOAD`
[#218](https://github.com/gaoyu06/native-obfuscator/pull/218)
(parent re-ran 304/304: 297 `IrCompilerTest` + 7 `CodegenModeTest`,
including `threeImmediateLnegSuperReturnsCompileAndRunWithJavaParity`)
on the post-[#217](https://github.com/gaoyu06/native-obfuscator/pull/217)
leaf-only `LDIV`/`LREM` tree. Active process:
...
- **Opt-in IR.** `--codegen=ir` lowers admitted methods through a typed CFG
  ...
  (five-or-more nested int binaries, nested long
  computed inputs, float/double/reference computed inputs),
...
- **(a) Scope / 范围:** Status refresh after landing #218 (one `LNEG`
  over a declared long `LLOAD`). /
  落地 #218 之后的现状刷新。
