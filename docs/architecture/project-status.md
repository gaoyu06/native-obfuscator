...
Last updated after landing well-formed `jsr`/`ret` inlining
[#241](https://github.com/gaoyu06/native-obfuscator/pull/241)
(parent re-ran 387/387: 380 `IrCompilerTest` + 7 `CodegenModeTest`,
including `jsrRetSubroutineCompileAndRunWithJavaParity`)
on the post-[#240](https://github.com/gaoyu06/native-obfuscator/pull/240)
five-level chain-input trees. Active process:
...
- Admit remaining IR leftovers so methods stop falling back:
  ...
  subroutines are admitted by #241.
...
- **(a) Scope / 范围:** Status refresh after landing #241 (well-formed
  `jsr`/`ret` inlining). /
  落地 #241 之后的现状刷新。
