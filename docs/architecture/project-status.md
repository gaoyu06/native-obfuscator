...
Last updated after landing leaf-only `LDIV`/`LREM` long chain inputs
[#217](https://github.com/gaoyu06/native-obfuscator/pull/217)
(parent re-ran 301/301: 294 `IrCompilerTest` + 7 `CodegenModeTest`,
including `threeImmediateLdivLremSuperReturnsCompileAndRunWithJavaParity`)
on the post-[#216](https://github.com/gaoyu06/native-obfuscator/pull/216)
leaf-only long shift tree. Active process:
...
- **Opt-in IR.** `--codegen=ir` lowers admitted methods through a typed CFG
  ...
  (five-or-more nested int binaries, nested/`LNEG`/other
...
- Admit remaining IR leftovers so methods stop falling back:
  ...
  int binaries, nested/`LNEG`/other long
...
- **(a) Scope / 范围:** Status refresh after landing #217 (leaf-only
  `LDIV`/`LREM` long chain inputs). /
  落地 #217 之后的现状刷新。
