...
Last updated after landing two-level nested float chain inputs
[#226](https://github.com/gaoyu06/native-obfuscator/pull/226)
(parent re-ran 329/329: 322 `IrCompilerTest` + 7 `CodegenModeTest`,
including `twoLevelNestedFloatChainInputsCompileAndRunWithJavaParity`)
on the post-[#225](https://github.com/gaoyu06/native-obfuscator/pull/225)
`FNEG` float leaf. Active process:
...
- **Opt-in IR.** `--codegen=ir` lowers admitted methods through a typed CFG
  ...
  binaries, three-or-more nested float binaries, extra-local float operands,
...
- Admit remaining IR leftovers so methods stop falling back:
  ...
  binaries, three-or-more nested float binaries, extra-local
...
- **(a) Scope / 范围:** Status refresh after landing #226 (two-level
