...
Last updated after landing two-level nested long chain inputs
[#219](https://github.com/gaoyu06/native-obfuscator/pull/219)
(parent re-ran 307/307: 300 `IrCompilerTest` + 7 `CodegenModeTest`,
including `twoLevelNestedLongChainInputsCompileAndRunWithJavaParity`)
on the post-[#218](https://github.com/gaoyu06/native-obfuscator/pull/218)
`LNEG` long-leaf tree. Active process:
...
- **Opt-in IR.** `--codegen=ir` lowers admitted methods through a typed CFG
  ...
  (five-or-more nested int binaries, three-or-more nested long
...
- Admit remaining IR leftovers so methods stop falling back:
  ...
  int binaries, three-or-more nested long
...
- **(a) Scope / 范围:** Status refresh after landing #219 (two-level
