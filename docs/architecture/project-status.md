...
Last updated after landing single-chain receiver-alias forwarding
[#202](https://github.com/gaoyu06/native-obfuscator/pull/202)
(parent re-ran 254/254: 247 `IrCompilerTest` + 7 `CodegenModeTest`,
including `receiverAliasForwardingCompilesAndRunsWithJavaParity`)
on the post-[#201](https://github.com/gaoyu06/native-obfuscator/pull/201)
relocated-handler tree. Active process:
...
- **Opt-in IR.** `--codegen=ir` lowers admitted methods through a typed CFG
  ...
  proven alias. Multi-call `ASTORE 0` aliasing, unproven
...
- Admit remaining IR leftovers so methods stop falling back:
  ...
  aliasing on diamond / identical-copy / path-id forms, unproven
  prefix→suffix jumps/switches, other mixed try/catch placements
...
- **(a) Scope / 范围:** Status refresh after landing #202 (single-chain
