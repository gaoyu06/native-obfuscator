...
Last updated after landing three-level nested float chain inputs
[#227](https://github.com/gaoyu06/native-obfuscator/pull/227)
(parent re-ran 332/332: 325 `IrCompilerTest` + 7 `CodegenModeTest`,
including `threeLevelNestedFloatChainInputsCompileAndRunWithJavaParity`)
on the post-[#226](https://github.com/gaoyu06/native-obfuscator/pull/226)
two-level nested float tree. Active process:
...
- Admit remaining IR leftovers so methods stop falling back:
  ...
  binaries, four-or-more nested float binaries, extra-local
...
- **(a) Scope / 范围:** Status refresh after landing #227 (three-level
