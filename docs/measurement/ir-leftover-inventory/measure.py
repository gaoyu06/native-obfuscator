...
def render_report(
    ...
    lines.extend(
        [
            ...
            "| --- | --- | --- |",
            "| Constructor `ASTORE 0` in retained prefix | "
            "`ConstructorSpecialMethodProcessor` | "
            "`Constructor prefix changes local 0 before the bridge` "
            "(opcode 58 / `ASTORE`). |",
            "| Constructor prefix → suffix branch | "
            "`ConstructorSpecialMethodProcessor` | "
            "`Constructor prefix branches across the this/super call` "
            "(jump/table/lookup-switch target outside retained prefix). |",
            "| Constructor cross-split catch region | "
            "`ConstructorSpecialMethodProcessor` | "
            "`Constructor exception regions may not cross the this/super split`. |",
            "| Multiple direct `this`/`super` candidates | "
            "`ConstructorSpecialMethodProcessor` | "
            "`Constructor has multiple possible this/super calls` "
            "(opcode 183 / `INVOKESPECIAL`). |",
            "| Unforwarded constructor-prefix extra local | "
            "`ConstructorSpecialMethodProcessor` → `AsmToIr` | "
            "The bridge forwards initialized `this` plus declared constructor "
            "arguments, not other prefix-created locals; a suffix read reaches "
            "`Read of a local not defined on every incoming edge` with the read opcode. |",
            "| Unsafe/unproven `ConstantDynamic` shape | "
            "`DynamicConstantSupport` | Rejected before resolver installation; "
            "exact source messages are listed below. |",
