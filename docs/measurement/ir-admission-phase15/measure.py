#!/usr/bin/env python3
"""Reproduce phase-15 IR admission measurements without changing compiler code."""
...
REQUIRED_TIP = "f46c3eae27f03071a8b3a9e161533b8f24c23735"
...
def main() -> int:
    ...
    parser.add_argument(
        "--work-dir",
        type=Path,
        default=Path("/tmp/native-obfuscator-ir-admission-phase15"),
    )
    parser.add_argument(
        "--include-jdk21",
        action="store_true",
    ...
    try:
        ...
        if base != REQUIRED_TIP:
            raise RuntimeError(
                f"expected phase-15 base {REQUIRED_TIP}, found merge-base {base}"
            )
