#!/usr/bin/env python3
"""Reproduce phase-18 IR admission measurements without changing compiler code."""
...
REQUIRED_TIP = "b78d6d7c74b11c416f5703df89ad6b0c1532aec2"
...
def main() -> int:
    ...
    parser.add_argument(
        "--work-dir",
        type=Path,
        default=Path("/tmp/native-obfuscator-ir-admission-phase18"),
    )
    parser.add_argument(
        "--include-jdk21",
        action="store_true",
    ...
    try:
        ...
        if base != REQUIRED_TIP:
            raise RuntimeError(
                f"expected phase-18 base {REQUIRED_TIP}, found merge-base {base}"
            )
