#!/usr/bin/env python3
"""Reproduce phase-17 IR admission measurements without changing compiler code."""
...
REQUIRED_TIP = "5a6f6097524c1fe42cd82be2425f5e6736667688"
...
def main() -> int:
    ...
    parser.add_argument(
        "--work-dir",
        type=Path,
        default=Path("/tmp/native-obfuscator-ir-admission-phase17"),
    )
    parser.add_argument(
        "--include-jdk21",
        action="store_true",
    ...
    try:
        ...
        if base != REQUIRED_TIP:
            raise RuntimeError(
                f"expected phase-17 base {REQUIRED_TIP}, found merge-base {base}"
            )
