...
7. The node type indexes a complete 16-slot ASM handler table: opcode nodes and metadata nodes have separate handlers (`MethodProcessor.java:55-79`). `GenericInstructionHandler` derives the opcode name, current source line, catch fragment, return type, and stack-index tokens, lets the concrete handler add tokens/prefix code, then expands the selected snippet (`GenericInstructionHandler.java:19-61`).
