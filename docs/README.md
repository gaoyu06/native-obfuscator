...
| [architecture/project-status.md](architecture/project-status.md) | EN + 中文 | What is on `master` after #118–#224. Claims that are **not** allowed. |
...
| [architecture/ir-flex-ctor-status.md](architecture/ir-flex-ctor-status.md) | Constructor split through hybrid suffixes, relocated and method-end handlers, receiver-alias forwarding, identical-copy extras/catch, four-level int-family and long chain inputs, and leaf-only float `FADD`/`FSUB`/`FMUL`/`FDIV`/`FREM` (#146/#160/#163–#166/#170–#224) |
