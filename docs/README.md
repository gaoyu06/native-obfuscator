...
| [architecture/project-status.md](architecture/project-status.md) | EN + 中文 | What is on `master` after #118–#406. Claims that are **not** allowed. |
...
| [architecture/ir-flex-ctor-status.md](architecture/ir-flex-ctor-status.md) | Constructor split through hybrid suffixes, relocated and method-end handlers, receiver-alias forwarding, identical-copy extras/catch, sixteen-level int-family/long/float/double chain inputs, prefix extra-local int/float/double/long leaves, extra-local long shift values plus `LDIV`/`LREM` operands, extra-local long `LNEG`, retained-prefix `AALOAD` chain inputs, extra-local int long-shift counts, extra-local float `FNEG`, extra-local double `DNEG`, extra-local int `INEG`, declared-array `IALOAD` int leaves, extra-local array `AALOAD` sources, extra-local `int[]` `IALOAD` sources, declared-array `BALOAD`/`CALOAD`/`SALOAD` leaves, `AALOAD` declared or extra-local `ILOAD` indexes, int-family array-load `ILOAD` indexes, extra-local `BALOAD`/`CALOAD`/`SALOAD` sources, declared `LALOAD`/`FALOAD`/`DALOAD` leaves, extra-local `LALOAD`/`FALOAD`/`DALOAD` sources, `LALOAD`/`FALOAD`/`DALOAD` `ILOAD` indexes, extra-array plus extra-index composition, the fail-closed prefix→suffix crossing audit, wide extra-array plus extra-index, the fail-closed unassigned-extra audit, the fail-closed spanning-catch audit, declared-argument `GETFIELD`, the fail-closed nine-path audit, isolated no-arg `NEW`, the fail-closed skip-super audit, the fail-closed seventeen-level audit, extra-local `GETFIELD` holders, the fail-closed unproven-`NEW` audit, primitive extra-local `GETFIELD` holders, the fail-closed unproven-`GETFIELD` audit, isolated one-arg `NEW`, the fail-closed post-call audit, isolated two-arg `NEW`, isolated three-arg `NEW`, extra-local int-copy `NEW` initializer, isolated four-arg `NEW`, the fail-closed unproven extra-array `AALOAD` audit, isolated five-arg `NEW`, the fail-closed constructor `jsr`/`ret` exception-table audit, the fail-closed unproven wide NEW initializer audit, isolated six-arg `NEW`, extra-local two-arg `NEW`, extra-local three-arg `NEW`, isolated long `NEW`, extra-local four-arg `NEW`, extra-local five-arg `NEW`, isolated float `NEW`, extra-local six-arg `NEW`, isolated double `NEW`, extra-local long `NEW`, extra-local float `NEW`, extra-local double `NEW`, extra-local float as second `NEW` argument, extra-local double as second `NEW` argument, extra-local float as both `NEW` arguments, and extra-local double as both `NEW` arguments, extra-local int as the second two-arg `NEW` argument, extra-local int as both two-arg `NEW` arguments, extra-local int as the second three-arg `NEW` argument, extra-local int as the third three-arg `NEW` argument, extra-local int as all three Color `NEW` arguments, extra-local int as the first and second Color `NEW` arguments, extra-local int as the first and third Color `NEW` arguments, extra-local int as the second and third Color `NEW` arguments, extra-local int as the second four-arg Insets `NEW` argument, extra-local int as the third four-arg Insets `NEW` argument, extra-local int as the fourth four-arg Insets `NEW` argument, extra-local int as all four Insets `NEW` arguments, extra-local int as the first and second Insets `NEW` arguments, extra-local int as the first and third Insets `NEW` arguments, extra-local int as the first and fourth Insets `NEW` arguments, extra-local int as the second and third Insets `NEW` arguments, extra-local int as the second and fourth Insets `NEW` arguments, extra-local int as the third and fourth Insets `NEW` arguments, extra-local int as the first, second, and third Insets `NEW` arguments, extra-local int as the first, second, and fourth Insets `NEW` arguments, extra-local int as the first, third, and fourth Insets `NEW` arguments, and extra-local int as the second, third, and fourth Insets `NEW` arguments, and extra-local int as the second five-arg GregorianCalendar `NEW` argument, and extra-local int as the third five-arg GregorianCalendar `NEW` argument, and extra-local int as the fourth five-arg GregorianCalendar `NEW` argument, and extra-local int as the fifth five-arg GregorianCalendar `NEW` argument, and extra-local int as all five five-arg GregorianCalendar `NEW` arguments, and extra-local int as the first and second five-arg GregorianCalendar `NEW` arguments, and extra-local int as the first and third five-arg GregorianCalendar `NEW` arguments, and extra-local int as the first and fourth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the first and fifth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the second and third five-arg GregorianCalendar `NEW` arguments, and extra-local int as the second and fourth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the second and fifth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the third and fourth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the third and fifth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the fourth and fifth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the first, second, and third five-arg GregorianCalendar `NEW` arguments, and extra-local int as the first, second, and fourth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the first, second, and fifth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the first, third, and fourth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the first, third, and fifth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the first, fourth, and fifth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the second, third, and fourth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the second, third, and fifth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the second, fourth, and fifth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the third, fourth, and fifth five-arg GregorianCalendar `NEW` arguments, and extra-local int as the first, second, third, and fourth five-arg GregorianCalendar `NEW` arguments (#146/#160/#163–#166/#170–#406) |
...
| [benchmarks/ir-leftover-inventory.md](benchmarks/ir-leftover-inventory.md) | Post-#403 admission inventory (#405; ClassicTest 108/108, JDK 17/21/25 82/82, 47/47, 21/21). Not coverage-complete |
